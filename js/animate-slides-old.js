;(function() {
	...
	Observer.prototype.update = function( context ) { 
		var n = this.context.indexOf(context); 
		this.open = this.show ? true : false;
		this.show = n === -1 ? false : true; 
	};
	...
	function addObservers() {
		...
		for ( i = 0; i < len; i++ ) {
			...
			settings.context = items[i].getAttribute('data-context');
	...
	function init() {
		...
		for ( i = 0; i < len; i++ ) {
			buttons[i].addEventListener('click', function () { 
				publisher.notify(this.getAttribute('data-context')); 
			}, false);
		}
		addObservers();
	}
